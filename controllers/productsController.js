...
controller.route('/').post(async (req,res) => {
    ...
    const item_exsists = await productSchema.findOne({name})
    if (item_exsists)
        ...
        if (product)
        res.status(201).json({text: `product with article number '${product._id}' added succsessfully`})
        else
            res.status(400).json({text: 'somthing went wrong when you tried to create the product'})
    }
})
...
controller.route('/:articleNumber').delete(async (req,res) => {
    if (!req.params.articleNumber)
        ...
        if (item) {
            ...
            res.status(200).json({text: `product with article number '${req.params.articleNumber}' deleted succsessfully.`})
        } else {
    ...
    if (!req.params.articleNumber) {
        ...
        const product = await productSchema.findById(req.params.articleNumber)
            if (product) {
                ...
                const updateProduct = await product.save({new: true})
                res.status(200).json({text: `product with article number '${req.params.articleNumber}' updated.`})
            } else {
